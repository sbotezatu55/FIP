...
The Angular application is located at `apps/fip-web` and currently provides the application shell, Index page, a live Flights list loaded from `GET /api/flights`, and a Flight Detail dashboard loaded from `GET /api/flights/{id}` and `GET /api/flights/{id}/summary`:
